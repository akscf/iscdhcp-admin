...
sub start {
	...
	for my $dir (@$mdirs) {
		_list_files(
			...
			sub {
				...
				unless ($minst) {
					die WSP::WspException->new("Couldn't load module: $@");
				}
				if ( !exists( $minst->{class_name} ) ) {
					die WSP::WspException->new("Couldn't identify class: $mfile");
				}
				...
				eval { $minst->init($self); } || do {
					my $exc = $@;
					die WSP::WspException->new( "Couldn't init module: "
	...
	foreach my $minst (@$modules) {
		...
		eval { $minst->start(); } || do {
			my $exc = $@;
			die WSP::WspException->new( "Couldn't start module: "
				  . $minst->get_class_name()
				  . ", error: "
				  . $exc );
		  }
...
sub rpc_service_register {
	...
	#
	unless ($service_name) {
		die WSP::WspException->new("Invalid argument: service_name");
	}
	unless ($instance) {
		die WSP::WspException->new("Invalid argument: instance");
	}
	#
...
sub servlet_register {
	...
	#
	unless ($path) {
		die WSP::WspException->new("Invalid argument: path");
	}
	unless ($instance) {
		die WSP::WspException->new("Invalid argument: instance");
	}
...
sub _list_dirs {
	...
	#
	opendir( DIR, $base )
	  || die WSP::WspException->new("Couldn't read directory: $!");
...
sub _list_files {
	my ( $self, $base, $cb ) = @_;
	#
	opendir( DIR, $base ) || die WSP::WspException->new("Couldn't read directory: $!");
	while ( my $file = readdir(DIR) ) {
		my $cfile = "$base/$file";
		$cb->($cfile) if ( -f $cfile );
	}
...
sub _pid_read {
	...
	#
	open( my $x, "<" . $self->{pid_file} ) || die WSP::WspException->new("Couldn't read pid file: $! (" . $self->{pid_file} . ")" );
	my $lpid = <$x>;
	close($x);
	chomp($lpid);
	#
...
sub _pid_write {
	my ($self) = @_;
	#
	open( my $x, ">" . $self->{pid_file} ) || die WSP::WspException->new("Couldn't write pid file: $! (" . $self->{pid_file} . ")" );
	print( $x "$$\n" );
	close($x);
}
