 ...
 */
qx.Class.define("fw.core.qooxdoo.widgets.XTable", {
    ...
    construct:function (tableModel, custom) {
        ...
        if(!tableModel) {
            throw new Error("tableModel is null");
        }
        if (!custom) {
            custom = { };
        }
    ...
    events:{
        ...
        "updateStatusbar"                   : "qx.event.type.Event"
    },
    ...
    members:{
        ...
        _createChildControlImpl:function (id, hash) {
            ...
            switch (id) {
                case "statusbar":
                    control = new qx.ui.container.Composite(new qx.ui.layout.HBox().set({spacing:1, alignX:'center'}, null));
                    control.set({
                        allowGrowX: true,
                        paddingLeft: 2,
                        paddingRight: 2
                    });
                    this._add(control, null);
                ...
                case "column-button":
                    ...
                    menu.addListener( "appear", this._initColumnMenu, this , false);
                    break;
            }
        ...
        _updateStatusBar:function () {
            if (this.getStatusBarVisible() && this.getStatusBarEventEnable()) {
                this.fireEvent("updateStatusbar");
            }
        },
        ...
        _initColumnMenu:function () {
            ...
            for (var col = 0, l = tableModel.getColumnCount(); col < l; col++) {
                var txt = tableModel.getColumnName(col);
                if(!txt || !txt.length) { txt = tableModel.getColumnId(col); }
                if(!txt || !txt.length || txt == "@") { continue; };

                var menuButton = columnButton.factory("menu-button", {
                    text:txt, column:col, bVisible:columnModel.isColumnVisible(col)
                });
                qx.core.Assert.assertInterface(menuButton, qx.ui.table.IColumnMenuItem);

                menuButton.addListener( "changeVisible",
        ...
        getStatusBar: function () {
            return this.getChildControl("statusbar");
        },

        statusBarAdd:function (widget, opts) {
            this.getChildControl("statusbar").add(widget, opts);
            return widget;
        },

        statusBarAddFlex:function (widget) {
            this.getChildControl("statusbar").add(widget, {flex:1});
            return widget;
        },

        statusBarAddSpacer:function() {
            var spacer = new qx.ui.core.Spacer;
            this.getChildControl("statusbar").add(spacer, {flex:1});
            return spacer;
        },

        statusBarAddSeparator : function() {
            // separator-vertical
            this.getChildControl("statusbar").add(new qx.ui.toolbar.Separator);
        },
