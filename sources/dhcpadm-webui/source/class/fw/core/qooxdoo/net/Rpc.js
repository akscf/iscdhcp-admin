 ...
 *  + firefox encode fixes
 *  + date parser
 *  + X-SESSION-ID (CSFR protection)
 * -----------------------------------------------------
 * Copyright (C) AlexandrinKS
 * https://akscf.me/
 */
qx.Class.define("fw.core.qooxdoo.net.Rpc", {
    ...
    members: {
        ...
        setParseJson: function (val) {
            this.__flParseJson = val;
        },
        ...
        _callInternal: function (args, callType, refreshSession) {
                ...
                switch (callType) {
                    ...
                    case 2: // async with event listeners, Dispatch the event to our listeners.
                        if (!ex) {
                            ...
                            ex.id = id;
                            if (args[0]) {      // coalesce
                                // They requested that we coalesce all failure types t "failed"
            ...
            var addToStringToObject = function (obj) {
                if (protocol == "qx1") {
                    obj.toString = function () {
                        switch (obj.origin) {
                            case fw.core.qooxdoo.net.Rpc.origin.server:
                                return "Server error " + obj.code + ": " + obj.message;
                            case fw.core.qooxdoo.net.Rpc.origin.application:
                                return "Application error " + obj.code + ": " + obj.message;
                            case fw.core.qooxdoo.net.Rpc.origin.transport:
                                return "Transport error " + obj.code + ": " + obj.message;
                            case fw.core.qooxdoo.net.Rpc.origin.local:
            ...
            req.addListener("failed", function (evt) {
                ...
                ex = makeException(fw.core.qooxdoo.net.Rpc.origin.transport, code, qx.io.remote.Exchange.statusCodeToString(code));
                id = this.getSequenceNumber();
                handleRequestFinished("failed", eventTarget);
            });

            req.addListener("timeout", function (evt) {
                ex = makeException(fw.core.qooxdoo.net.Rpc.origin.local, fw.core.qooxdoo.net.Rpc.localError.timeout, "Local time-out expired for " + whichMethod);
                id = this.getSequenceNumber();
                handleRequestFinished("timeout", eventTarget);
            });

            req.addListener("aborted", function (evt) {
                ex = makeException(fw.core.qooxdoo.net.Rpc.origin.local, fw.core.qooxdoo.net.Rpc.localError.abort, "Aborted " + whichMethod);
                id = this.getSequenceNumber();
            ...
            //
            req.setData(qx.lang.Json.stringify(rpcData, replacer, null));
